...
@click.command()
@click.option('--server', default="https://fec-dev-api.app.cloud.gov", help='server to check with.')
@click.option('--api_key', default=lambda: os.environ.get('FEC_API_KEY', ''), help='an api_key is needed for testing.')
def check_endpoints(server, api_key):
    """Simple script to check all endpoints status quickly."""
    for endp in endpoints:
        r = requests.get((server + '/v1' + endp).format(api_key=api_key,
