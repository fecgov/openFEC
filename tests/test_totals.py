...
class TestCandidateTotals(ApiBaseTest):
    ...
    candidate_committee_totals_fields = {
    ...
    house_senate_fields = {
        'all_other_loans': 460.0,
        'loan_repayments': 470.0,
        'loan_repayments_candidate_loans': 480.0,
        'loan_repayments_other_loans': 490.0,
        'loans': 500.0,
        'loans_made_by_candidate': 510.0,
        'transfers_from_other_authorized_committee': 520.0,
    ...
    def test_house_senate_totals_fields(self):
        ...
        first_candidate = utils.extend(self.house_senate_fields,
                                       self.candidate_committee_totals_fields,
                                       {'candidate_id': 'H0001',
                                        'cycle': 2020})

        second_candidate = utils.extend(self.house_senate_fields,
                                        self.candidate_committee_totals_fields,
                                        {'candidate_id': 'H0002',
                                         'cycle': 2022})
        factories.CandidateCommitteeTotalsHouseSenateFactory(**first_candidate)

        factories.CandidateCommitteeTotalsHouseSenateFactory(
            **second_candidate)

        results = self._results(
             api.url_for(CandidateTotalsView, candidate_id='H0001')
         )
        fields = utils.extend(self.candidate_committee_totals_fields,
                              self.house_senate_fields,
                              first_candidate)
    ...
    def test_presidential_totals_fields(self):
        ...
        first_candidate = utils.extend(self.candidate_committee_totals_fields,
                                       self.presidental_fields,
                                       {'candidate_id': 'P0001',
                                        'cycle': 2020})

        second_candidate = utils.extend(self.candidate_committee_totals_fields,
                                        self.presidental_fields,
                                        {'candidate_id': 'P0002',
                                         'cycle': 2022})
        factories.CandidateCommitteeTotalsPresidentialFactory(
            **first_candidate)
