...
class TestMonitoring(ApiBaseTest):
    ...
    @patch("webservices.tasks.utils.redis_instance")
    def test_set_redis_value(self, mock_redis):
        set_redis_value("FOO", {"bar": "baz"}, age=600)

        mock_redis.set.assert_called_once_with("FOO", json.dumps({"bar": "baz"}), ex=600)

    @patch("webservices.tasks.utils.redis_instance")
    def test_get_redis_value_found(self, mock_redis):
    ...
    @patch("webservices.tasks.utils.redis_instance")
    def test_get_redis_value_not_found(self, mock_redis):
    ...
    @patch("webservices.tasks.utils.redis_instance")
    def test_get_redis_value_empty(self, mock_redis):
