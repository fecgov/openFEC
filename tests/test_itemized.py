...
class TestItemized(ApiBaseTest):
    ...
    def test_null_pagination_with_null_sort_column_values_descending_with_sort_expression(
        self
    ):
        # NOTE:  Schedule B is sorted by disbursement date with the expression
        # sa.func.coalesce(self.disbursement_date, sa.cast('9999-12-31', sa.Date))
    ...
    def test_null_pagination_with_null_sort_column_values_ascending_with_sort_expression(
        self
    ):
        # NOTE:  Schedule B is sorted by disbursement date with the expression
        # sa.func.coalesce(self.disbursement_date, sa.cast('9999-12-31', sa.Date))
    ...
    def test_image_number_range(self):
        ...
        results = self._results(
            api.url_for(ScheduleAView, min_image_number='2', max_image_number='3')
        )
    ...
    def test_amount_sched_a(self):
        ...
        results = self._results(
            api.url_for(ScheduleAView, min_amount=100, max_amount=150)
        )
        self.assertTrue(
