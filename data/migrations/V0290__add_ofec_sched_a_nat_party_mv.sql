...
1) Create ofec_sched_a_national_party_mv which includes all contributions of national party accounts
*/

DROP MATERIALIZED VIEW IF EXISTS public.ofec_sched_a_national_party_mv;

CREATE MATERIALIZED VIEW IF NOT EXISTS public.ofec_sched_a_national_party_mv
AS
SELECT sa.cmte_id,
    v.name AS cmte_nm,
    v.committee_type,
...
ALTER TABLE IF EXISTS public.ofec_sched_a_national_party_mv OWNER TO fec;

GRANT ALL ON TABLE public.ofec_sched_a_national_party_mv TO fec;
GRANT SELECT ON TABLE public.ofec_sched_a_national_party_mv TO fec_read;

CREATE UNIQUE INDEX idx_ofec_sched_a_national_party_mv_subid
    ON public.ofec_sched_a_national_party_mv USING btree
    (sub_id);
CREATE INDEX idx_ofec_sched_a_national_party_mv_cln_contbr_id
    ON public.ofec_sched_a_national_party_mv USING btree
    (clean_contbr_id );
CREATE INDEX idx_ofec_sched_a_national_party_mv_contbr_city
    ON public.ofec_sched_a_national_party_mv USING btree
    (contbr_city);
CREATE INDEX idx_ofec_sched_a_national_party_mv_contbr_emp_text
    ON public.ofec_sched_a_national_party_mv USING gin
    (contributor_employer_text);
CREATE INDEX idx_ofec_sched_a_national_party_mv_contbr_name_text
    ON public.ofec_sched_a_national_party_mv USING gin
    (contributor_name_text);
CREATE INDEX idx_ofec_sched_a_national_party_mv_contbr_occ_text
    ON public.ofec_sched_a_national_party_mv USING gin
    (contributor_occupation_text);
CREATE INDEX idx_ofec_sched_a_national_party_mv_contbr_st
    ON public.ofec_sched_a_national_party_mv USING btree
    (contbr_st);
CREATE INDEX idx_ofec_sched_a_national_party_mv_contbr_zip
    ON public.ofec_sched_a_national_party_mv USING gin
    (contbr_zip);
CREATE INDEX idx_ofec_sched_a_national_party_mv_image_num
    ON public.ofec_sched_a_national_party_mv USING btree
    (image_num);
CREATE INDEX idx_ofec_sched_a_national_party_mv_rpt_yr
    ON public.ofec_sched_a_national_party_mv USING btree
    (rpt_yr);
