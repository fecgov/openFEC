/*
Issue #3549 Make new /candidates/totals/by_party/by_office endpoint

ofec_candidate_history_with_future_election_vw is the driver in the main query,
which already has all the additional candidate information needed:
office
party
candidate_active

Add these 3 fields in the list of select.
Note:  add these 3 fields at the end of the MV, 
otherwise the counterpart VW will need to be dropped and recreate, not create or replace.s
That will then cause the downstream mv/vw need to be drop/recreated
*/
...
-- It is noted fec_read missing select permission to these two MVs, added here 
grant select on ofec_amendments_mv to fec_read;
grant select on ofec_totals_candidate_committees_mv to fec_read;