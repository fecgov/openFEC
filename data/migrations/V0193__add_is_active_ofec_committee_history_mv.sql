/*
This migration file is for #4283
1) Add function public.is_committee_active(cmte_filing_freq text)
2) Add is_active column to ofec_committee_history_mv
*/

-- ----------
-- Function: public.is_committee_active
-- ----------
CREATE OR REPLACE FUNCTION public.is_committee_active(cmte_filing_freq text) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
    BEGIN
        RETURN CASE cmte_filing_freq
            WHEN 'Q' THEN TRUE
            WHEN 'M' THEN TRUE
            WHEN 'T' THEN FALSE
            WHEN 'A' THEN FALSE
...
ALTER FUNCTION public.is_committee_active(cmte_filing_freq text) OWNER TO fec;
