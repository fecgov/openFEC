...
Update presidential views to include 2008 and 2012 presidential tables
*/

-- ---------------------------------------------------
-- public.ofec_presidential_by_candidate_vw
-- previous migration file V0182/V0268
...
-- --------------
    ...
    FROM disclosure.pres_f3p_totals_ca_cm_link_12d
    UNION
    -- per-state totals
    SELECT DISTINCT
    cand_id AS candidate_id,
    ...
    contbr_st AS contributor_state,
    2012 AS election_year
    FROM disclosure.pres_ca_cm_sched_state_12d
    GROUP BY candidate_id,candidate_party_affiliation,candidate_last_name, contributor_state
-- --------------
    UNION
    -- 2008 totals
    ...
    FROM disclosure.pres_f3p_totals_ca_cm_link_08d
    UNION
    -- per-state totals
    SELECT DISTINCT
    cand_id AS candidate_id,
    ...
    contbr_st AS contributor_state,
    2008 AS election_year
    FROM disclosure.pres_ca_cm_sched_state_08d
    GROUP BY candidate_id,candidate_party_affiliation,candidate_last_name, contributor_state
--    
...
-- previous migration file V0183/V0268
...
-- previous migration file V0184/V0268
...
FROM (
    ...
    UNION
    --2012 data
    SELECT 2012 as election_year, *
    FROM disclosure.pres_f3p_totals_ca_cm_link_12d
    UNION
    --2008 data
    SELECT 2008 as election_year, *
    FROM disclosure.pres_f3p_totals_ca_cm_link_08d) AS combined
...
-- previous migration file V0185/V0268
...
-- previous migration file V0186/V0268
...
FROM (
    ...
    to_char(max(cvg_end_dt), 'mm/dd/yyyy'):: timestamp without time zone as coverage_end_date,
    2012 as election_year
    FROM disclosure.pres_nml_form_3p_12d
    GROUP BY candidate_id
    UNION
    -- 2008 data
    SELECT
    ...
    to_char(max(cvg_end_dt), 'mm/dd/yyyy'):: timestamp without time zone as coverage_end_date,
    2008 as election_year
    FROM disclosure.pres_nml_form_3p_08d
