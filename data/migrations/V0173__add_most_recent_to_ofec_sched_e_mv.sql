/*
This is to solve issue #4056 (API issue #4058).
Add the most_recent column to ofec_sched_e_mv.

ofec_sched_e_mv has 4 main source tables:
-form5 regular filing (regular report): data from fec_fitem tables. So it is most_recent)
-form5 notice (notice)
-form3X sched e (regular report): data from fec_fitem tables. So it is most_recent)
-form24 sched_e (notice)

For regularly filed reports (such as Q1, YE, etc), 
v_sum/f_item table has the most recent filing.  
Therefore the most_recent flag for rows from fec_fitem tables are set to "true"

For notices, we need to base on the amendment chain (file_num/prev_file_num).
However, for quite some pfiled reports, the file_num < 0 and this amendment chain mothod is not reliable.
And since they don't have a fixed file time, the receipt_dt/image_num based method can not be used neither.

Currently, we have several amendment chain related mv/vw.  
ofec_processed_financial_amendment_chain_vw, which only consist reports with file_num > 0 and is more reliable.

Fortunately, for sched_e notices, majority of the reports has file_num > 0.

Therefore this vw is chosen to be the source to determine the most_recent flag for the notices.  
If a report's file_num can not be find in ofec_processed_financial_amendment_chain_vw, 
then we can not determine if it is most recent or not, the most_recent is set to null

If its file_num is the same as the mst_rct_file_num, then most_recent = 'true'
If its file_num is the different from the mst_rct_file_num, then most_recent = 'false'
*/
