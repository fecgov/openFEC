...
/*
This migration is for issue #5416
Add coalesce to cmte_name in tsvector column
Previous mv is V0258
*/
-- -----------------------------------------------------
...
CREATE MATERIALIZED VIEW public.ofec_reports_presidential_mv_tmp AS
  SELECT row_number() OVER () AS idx,
    ...
    to_tsvector(parse_fulltext(coalesce(cmte.cmte_nm,'')||' '||f3p.cmte_id)) AS filer_name_text
...
ALTER TABLE public.ofec_reports_presidential_vw OWNER TO fec;
GRANT ALL ON TABLE public.ofec_reports_presidential_vw TO fec;
GRANT SELECT ON TABLE public.ofec_reports_presidential_vw TO fec_read;
