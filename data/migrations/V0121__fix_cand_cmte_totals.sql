/*
Fix full cycle totals for ofec_totals_candidate_committees_mv
Issue #https://github.com/fecgov/fec-cms/issues/2631

- 1) Fix logic for ofec_totals_candidate_committees_vw
    a) `create or replace ofec_totals_candidate_committees_vw` to use new `MV` logic
    b) drop old `MV`
...
CREATE OR REPLACE VIEW public.ofec_totals_candidate_committees_vw AS
WITH last_cycle AS (
         SELECT DISTINCT ON (v_sum.cmte_id, link.fec_election_yr) v_sum.cmte_id,
            ...
            link.cand_election_yr + link.cand_election_yr % 2 AS election_year
         ...
         SELECT DISTINCT ON (link.cand_id, link.fec_election_yr) link.cand_id AS candidate_id,
            link.fec_election_yr AS cycle,
            max(link.cand_election_yr + link.cand_election_yr % 2) AS election_year,
