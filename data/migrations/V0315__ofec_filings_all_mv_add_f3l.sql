/*
For issue # 6375
Add F3L to the logic for most_recent column
Previous version is V0310
*/
-- ----------
...
SELECT row_number() OVER () AS idx,
	...
	CASE
		WHEN upper(filing_history.form_tp) IN ('FRQ', 'F99') THEN 0
	...
	CASE
		...
		WHEN upper(filing_history.form_tp::text) in ('F24','F3L') THEN 
	    	CASE 
	...
	CASE
		WHEN upper(filing_history.form_tp) in ('F3', 'F3X', 'F3P', 'F3L', 'F4', 'F5', 'F7', 'F13', 'F24') THEN 'REPORT'::varchar(10)   
		WHEN upper(filing_history.form_tp) in ('F6', 'F9', 'F10', 'F11') THEN 'NOTICE'::varchar(10)   
		WHEN upper(filing_history.form_tp) in ('F1','F2') THEN 'STATEMENT'::varchar(10)    
		WHEN upper(filing_history.form_tp) in ('F1M', 'F8', 'F99', 'F12', 'FRQ') THEN 'OTHER'::varchar(10)   
    ...
    to_tsvector(parse_fulltext(coalesce(com.name,'')||' '||filing_history.cand_cmte_id)) AS filer_name_text
...
LEFT JOIN ofec_filings_amendments_all_vw amendments ON filing_history.cand_cmte_id = amendments.cand_cmte_id AND filing_history.file_num = amendments.file_num
...
WHERE filing_history.rpt_yr >= 1979 AND filing_history.form_tp::text <> 'SL'::text AND filing_history.form_tp::text <> 'SI'::text
WITH DATA;
...
CREATE INDEX idx_ofec_filings_all_mv_tmp_form_category
    ON public.ofec_filings_all_mv_tmp USING btree
    (form_category);  
CREATE INDEX idx_ofec_filings_all_mv_tmp_filer_name_text
    ON public.ofec_filings_all_mv_tmp USING gin (filer_name_text);
...
ALTER INDEX IF EXISTS idx_ofec_filings_all_mv_tmp_filer_name_text RENAME TO idx_ofec_filings_all_mv_filer_name_text;
