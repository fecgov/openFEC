...
Since currently the model file is directly point to the mv itself, 
the _tmp approach is used to minimize the the downtime.  

This PR also to change the candidate.py model file to point to its front-end view (ofec_candidate_history_vw).
So next time it does not need to use the _tmp approach.

There are still some MV dependency that were not handled berfore.
