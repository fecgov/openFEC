/*
This migration file is for #4034
Add 4 columns to ofec_committee_history_mv to help filter committees activity to 
only shows fec_elecrion_yr with financial report/notice 
or only shows fec_elecrion_yr with filings
...
WITH cycles AS (
         ...
         SELECT f_rpt_or_form_sub.cand_cmte_id AS cmte_id,
            ...
            max(get_cycle(rpt_yr)) as last_cycle_has_activity,
            array_agg(DISTINCT(get_cycle(rpt_yr)))::integer[] AS cycles_has_activity
        ...
        select cand_cmte_id as cmte_id,
        max(get_cycle(rpt_yr)) as last_cycle_has_financial,
        array_agg(DISTINCT(get_cycle(rpt_yr)))::integer[] AS cycles_has_financial
        from disclosure.f_rpt_or_form_sub
        where upper(form_tp) in ('F3', 'F3X', 'F3P', 'F3L', 'F4', 'F5', 'F7', 'F13','F24', 'F6', 'F9', 'F10', 'F11')
 ...
 SELECT DISTINCT ON (fec_yr.cmte_id, fec_yr.fec_election_yr) row_number() OVER () AS idx,
   ...
   FROM ((((disclosure.cmte_valid_fec_yr fec_yr
     ...
     LEFT JOIN reports ON fec_yr.cmte_id = reports.cmte_id
  WHERE ((cycles.max_cycle >= (1979)::numeric) AND (NOT ((fec_yr.cmte_id)::text IN 
