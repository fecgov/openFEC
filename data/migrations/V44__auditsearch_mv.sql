...
--
-- 1)Name: ofec_audit_case_mv; Type: MATERIALIZED VIEW; Schema: auditsearch; Owner: fec
--
...
audit_case_sebset AS (
    SELECT 
        ...
        election_cycle::integer AS cycle,
        audit_case.cmte_id AS committee_id,
        cmte_info.cmte_nm AS committee_name,
        cmte_info.cmte_dsgn AS committee_designation,
        cmte_info.cmte_tp AS committee_type,
        cmte_info.cmte_desc AS committee_description,
        audit_case.far_release_date::date AS far_release_date,
        audit_case.link_to_report,
        audit_case.audit_id::integer AS audit_id,
        audit_case.cand_id AS candidate_id,
        coalesce(cand_info.cand_name, 'No authorized candidate'::text) AS candidate_name
    FROM audit_case_finding_info
    LEFT JOIN audit_case ON (audit_case_finding_info.audit_case_id= audit_case.audit_case_id::integer)
...
FROM audit_case_finding acf 
LEFT JOIN finding pf ON (acf.parent_finding_pk = pf.finding_pK) 
LEFT JOIN finding cf ON (acf.child_finding_pk = cf.finding_pK) 
...
from cmte_audit_vw order by cmte_id;
...
from cand_audit_vw order by cand_id;
