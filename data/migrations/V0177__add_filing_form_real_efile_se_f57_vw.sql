...
CREATE OR REPLACE VIEW public.real_efile_se_f57_vw AS 
 ...
 SELECT combined.repid,
    ...
    CASE
        ...
        ELSE false
    END AS most_recent,
    to_tsvector(concat(parse_fulltext(combined.so_can_name::text), ' ', parse_fulltext(combined.so_can_fname::text), ' ', combined.so_canid::text)) AS cand_fulltxt,
    CASE 
        WHEN rep.form like 'F3X%' THEN 'F3X'::text
        WHEN rep.form like 'F5%' THEN 'F5'::text
        WHEN rep.form like 'F24%' THEN 'F24'::text
        ELSE rep.form
    END AS filing_form,
    CASE 
       WHEN rep.form like 'F24%' 
        OR (rep.form like 'F5%' AND rep.rptcode is null) THEN true
       ELSE false
    END AS is_notice
   FROM combined
     LEFT JOIN real_efile.reps rep ON combined.repid = rep.repid 
