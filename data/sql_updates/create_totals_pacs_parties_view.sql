...
select
    ...
    sum(coalesce(pnp.ttl_contb_ref_per_i, pnp.ttl_contb_ref_per_ii)) as contribution_refunds,
    sum(pnp.ttl_contb_per) as contributions,
    sum(pnp.coord_exp_by_pty_cmte_per) as coordinated_expenditures_by_party_committee,
    sum(coalesce(pnp.ttl_disb_sum_page_per, pnp.ttl_disb_per)) as disbursements,
    ...
    sum(coalesce(pnp.offsets_to_op_exp_per_i, pnp.offsets_to_op_exp_per_ii)) as offsets_to_operating_expenditures,
    ...
    sum(coalesce(pnp.ttl_receipts_sum_page_per, pnp.ttl_receipts_per)) as receipts,
