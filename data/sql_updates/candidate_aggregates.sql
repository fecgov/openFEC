drop table if exists ofec_candidate_totals_tmp;
create table ofec_candidate_totals_tmp as
...
cycle_totals as (
    ...
    from ofec_cand_cmte_linkage link
    join totals on
        link.cmte_id = totals.committee_id and
        link.fec_election_yr = totals.cycle
    left join ofec_candidate_election election on
...
create unique index on ofec_candidate_totals_tmp (candidate_id, cycle, is_election);

create index on ofec_candidate_totals_tmp (candidate_id);
create index on ofec_candidate_totals_tmp (election_year);
create index on ofec_candidate_totals_tmp (cycle);
create index on ofec_candidate_totals_tmp (is_election);
create index on ofec_candidate_totals_tmp (receipts);
create index on ofec_candidate_totals_tmp (disbursements);

drop table if exists ofec_candidate_totals;
alter table ofec_candidate_totals_tmp rename to ofec_candidate_totals;
