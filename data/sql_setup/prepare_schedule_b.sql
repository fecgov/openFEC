...
create trigger nml_sched_b_after_trigger after insert or update
    on disclosure.nml_sched_b for each row execute procedure ofec_sched_b_insert_update(:START_YEAR_AGGREGATE);
...
create trigger nml_sched_b_before_trigger before delete or update
    on disclosure.nml_sched_b for each row execute procedure ofec_sched_b_delete_update(:START_YEAR_AGGREGATE);
...
create trigger f_item_sched_b_after_trigger after insert or update
    on disclosure.f_item_receipt_or_exp for each row execute procedure ofec_sched_b_insert_update(:START_YEAR_AGGREGATE);
