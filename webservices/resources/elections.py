...
class ElectionsListView(utils.Resource):
    ...
    filter_multi_fields = [('cycle', ElectionsList.cycle)]
...
class ElectionView(ApiResource):
    ...
    def args(self):
        return utils.extend(
            args.paging, args.elections, args.make_sort_args(default='-total_receipts')
        )
    ...
    def build_query(self, **kwargs):
        ...
        return (
            ...
            .outerjoin(latest, aggregates.c.candidate_id == latest.c.candidate_id)
            .outerjoin(
                BaseConcreteCommittee,
    ...
    def _get_latest(self, pairs):
        latest = (
            db.session.query(pairs.c.cash_on_hand_end_period, pairs.c.candidate_id)
            .distinct(pairs.c.candidate_id, pairs.c.cmte_id)
            .order_by(
                pairs.c.candidate_id, pairs.c.cmte_id, sa.desc(pairs.c.two_year_period)
            )
            .subquery()
        )
        return db.session.query(
            latest.c.candidate_id,
            sa.func.sum(sa.func.coalesce(latest.c.cash_on_hand_end_period, 0.0)).label(
                'cash_on_hand_end_period'
            ),
        ).group_by(latest.c.candidate_id)
    ...
    def _get_aggregates(self, pairs):
        return db.session.query(
            ...
            sa.func.sum(sa.func.coalesce(pairs.c.receipts, 0.0)).label(
                'total_receipts'
            ),
            sa.func.sum(sa.func.coalesce(pairs.c.disbursements, 0.0)).label(
                'total_disbursements'
            ),
            sa.func.sum(sa.func.coalesce(pairs.c.cash_on_hand_end_period, 0.0)).label(
                'cash_on_hand_end_period'
            ),
        ...
        ).group_by(pairs.c.candidate_id, pairs.c.candidate_election_year)
...
election_durations = {'senate': 6, 'president': 4, 'house': 2}
...
def filter_candidates(query, kwargs):
    ...
    query = query.filter(
        ...
        CandidateHistory.candidate_election_year
        + (CandidateHistory.candidate_election_year % 2)
        == kwargs['cycle'],
...
@doc(tags=['filer resources'], description=docs.STATE_ELECTION_OFFICES)
class StateElectionOfficeInfoView(ApiResource):
    ...
    filter_match_fields = [('state', models.StateElectionOfficeInfo.state)]
