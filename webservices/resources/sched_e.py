...
@doc(
    tags=['independent expenditures'],
    description=docs.SCHEDULE_E_NOTICE,
)
class ScheduleENoticeView(ItemizedResource):

    model = models.ScheduleENotice
    schema = schemas.ScheduleENoticeSchema
    page_schema = schemas.ScheduleENoticePageSchema
    ...
    filter_multi_fields = [
        ('cycle', sa.func.get_cycle(models.ScheduleENotice.report_year)),
        ('image_number', models.ScheduleENotice.image_number),
        ('committee_id', models.ScheduleENotice.committee_id),
        ('candidate_id', models.ScheduleENotice.candidate_id),
        ('support_oppose_indicator', models.ScheduleENotice.support_oppose_indicator),
        ('filing_form', models.ScheduleENotice.filing_form),
        ('is_notice', models.ScheduleENotice.is_notice),
    ]
    filter_fulltext_fields = [
        ('payee_name', models.ScheduleENotice.payee_name_text),
    ]
    filter_range_fields = [
        (('min_date', 'max_date'), models.ScheduleENotice.expenditure_date),
        (('min_amount', 'max_amount'), models.ScheduleENotice.expenditure_amount),
        (('min_image_number', 'max_image_number'), models.ScheduleENotice.image_number),
    ]
    query_options = [
        sa.orm.joinedload(models.ScheduleENotice.candidate),
        sa.orm.joinedload(models.ScheduleENotice.committee),
    ]
    ...
    def filter_election(self, query, kwargs):
        ...
        query = query.join(
            models.CandidateHistory,
            models.ScheduleENotice.candidate_id == models.CandidateHistory.candidate_id,
        ).filter(
            models.CandidateHistory.two_year_period == kwargs['cycle'],
            models.CandidateHistory.office == kwargs['office'][0].upper(),
            models.ScheduleENotice.report_year.in_([kwargs['cycle'], kwargs['cycle'] - 1]),
        )
        if kwargs['state']:
            query = query.filter(models.CandidateHistory.state == kwargs['state'])
        if kwargs['district']:
