...
def build_exclude_list(value_list):
    exclude_list = [
        parse_exclude_arg(value) for value in value_list if is_exclude_arg(value)
    ]
    return exclude_list
    ...
    for key, column in fields:
        if kwargs.get(key) is not None:
            if is_exclude_arg(kwargs[key]):
                query = query.filter(
                    sa.or_(column != parse_exclude_arg(kwargs[key]), column == None)  # noqa: sqlalchemy quirk
                )
            else:
...
def filter_multi(query, kwargs, fields):
    for key, column in fields:
        if kwargs.get(key):
            ...
            if exclude_list:
                query = query.filter(
                    sa.or_(column.notin_(exclude_list), column == None)  # noqa: sqlalchemy quirk
                )
            if include_list:
...
def filter_fulltext(query, kwargs, fields):
    for key, column in fields:
        if kwargs.get(key):
            ...
            if include_list:
                filters = [
                    column.match(utils.parse_fulltext(value)) for value in include_list
                ]
...
def filter_multi_start_with(query, kwargs, fields):
    for key, column in fields:
        if kwargs.get(key):
            ...
            include_list = build_include_list(kwargs.get(key))
            if exclude_list:
                filters = [sa.not_(column.startswith(value)) for value in exclude_list]
                query = query.filter(sa.and_(*filters))
            if include_list:
                filters = [column.startswith(value) for value in include_list]
                query = query.filter(sa.or_(*filters))
    return query
...
def filter_election(
    query, kwargs, candidate_column, cycle_column=None, year_column=None
):
    ...
    if isinstance(kwargs['cycle'], list):
        if len(kwargs['cycle']) != 1:
            raise exceptions.ApiError(
                'Must include exactly one argument "cycle"', status_code=422
            )
