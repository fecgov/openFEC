...
class CandidateCommitteeLink(db.Model):
    __tablename__ = 'ofec_cand_cmte_linkage'

    linkage_id = db.Column(db.Integer, primary_key=True)
    committee_id = db.Column('cmte_id', db.String, db.ForeignKey('ofec_committee_detail.committee_id'))
    candidate_id = db.Column('cand_id', db.String, db.ForeignKey('ofec_candidate_detail.candidate_id'))
