...
class CommitteeReports(FecFileNumberMixin, PdfMixin, CsvMixin, BaseModel):
    ...
    is_amended = db.Column(
        'is_amended',
        db.Boolean,
        doc='False indicates that a report is the most recent. True indicates that the report has been superseded by an amendment.',
    )
...
class CommitteeReportsHouseSenate(CommitteeReports):
    ...
    total_contribution_refunds_col_total_period = db.Column(
        db.Numeric(30, 2)
