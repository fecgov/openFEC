...
class ReportType(db.Model, StagingMixin):
    ...
    report_type = db.Column(
        'rpt_tp_cd', db.String, index=True, primary_key=True, doc=docs.REPORT_TYPE
    )
    report_type_full = db.Column(
        'rpt_tp_desc', db.String, index=True, doc=docs.REPORT_TYPE
    )
...
class ReportDate(db.Model, DateMixin, FecAppMixin):
    ...
    report_type = db.Column(
        db.String, db.ForeignKey(ReportType.report_type), index=True
    )
...
class ElectionDate(db.Model, FecAppMixin):
    ...
    election_type_id = db.Column(
        'trc_election_type_id', db.String, index=True, doc=docs.ELECTION_TYPE
    )
    ...
    election_year = db.Column(
        'election_yr', db.Integer, index=True, doc=docs.ELECTION_YEAR
    )
    # I think this is mislabeled
    primary_general_date = db.Column('pg_date', db.Date, index=True)
    election_status_id = db.Column(
        'trc_election_status_id',
        db.Integer,
        index=True,
        doc='Records are disregarded if election status is not 1. Those records are erroneous.',
    )
...
class ElectionClassDate(db.Model):
    ...
    open_seat_flag = db.Column(
        'open_seat_flg',
        db.String,
        doc='Signifies if the contest has no incumbent running',
    )
...
class CalendarDate(db.Model):
    ...
    event_id = db.Column(
        'cal_event_id', db.Integer, primary_key=True, doc=docs.EVENT_ID
    )
