...
def get_single_case(case_type, case_no):
    ...
    with db.engine.connect() as conn:
        ...
        if row is not None:
            ...
            logger.info("TEST get_single_case before single  case  return: {0} {1}".format(case_type, case_no))
            return case
        else:
...
def get_documents(case_id, bucket, bucket_name):
    ...
    with db.engine.connect() as conn:
        ...
        if rs is None:
            logger.info("TEST get_documents NONE ResultSet: {0}".format(case_id))
        ...
        for row in rs:
            ...
            logger.info("TEST get_documents result DOCUMENT 1: {0}".format(case_id)) # NO MUR case id 60000002999600
