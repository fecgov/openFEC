...
class BaseCommitteeResource(Committee):
    """Common Base Class for Committee Resources. Not a Mixin as the resources
        ...
        for cmte in data:
            ...
            for item in cmte['dimcmteproperties']:
                ...
                for api_name, fec_name in \
                        self.committee_address_field_mappings:
                ...
                if len(description) > 0:
                    ...
                    if not expired and len(description) > 0:
                        ...
                        if 'description' not in record:
                            record['description'] = {}
                        record['description'][item['expire_date']] = \
                            description
                ...
                for designation in cmte['dimcmtetpdsgn']:
                    ...
                    if ('cmte_dsgn' in designation and
                            designation['cmte_dsgn'] in decoders.designation):
                    ...
                    if ('cmte_tp' in designation
                        ...
                        status['type_full'] = \
                            decoders.cmte[designation['cmte_tp']]
                    ...
                    if len(status) > 0:
                        if designation['expire_date'] is None:
                            ...
                            if 'status' not in record:
                                record['status'] = {}
                            record['status'][designation['expire_date']] = \
                                status
            ...
            # if there are no current records, add the most recent record to
            # the top level committee information
            for record_type in record:
                if record_type not in committee:
                    key = sorted(record[record_type],
                                 key=record[record_type].get,
                                 reverse=True)[0]
            ...
            # name short cut
            if ('description' in committee
                    and 'name' in committee['description']):
